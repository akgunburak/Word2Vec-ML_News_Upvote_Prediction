...
model = gensim.models.Word2Vec(list(dataset["title"]),
                               size=300,
                               min_count = 1)
...
def feature_engineering(dataset):
    ...
    dataset["title"] = dataset["title"].map(lambda x: [word for word in x if word in model.wv.vocab])
...
scores.plot(ax=ax, kind="bar",
            ...
            legend=None)
plt.savefig('results/regression_wv_results.png')
